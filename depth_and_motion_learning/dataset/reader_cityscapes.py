...
def read_frame_sequence_from_data_path(train_file_path,
  ...
  directory = os.path.dirname(train_file_path) # data
  ...
  def make_filename(line):
    ...
    if len(words) != 2:
      raise RuntimeError('Invalid train file: Each lines are expected to have '
                         ...
                         'conform.' % line)
    return os.path.join(directory, words[0].strip(), words[1].strip())

  files = [make_filename(line) for line in lines] # [0000000001, 000000002, ..., 0000000103]
